...
class RouterBuilder {
    ...
    static def buildRouter(Vertx vertx, File routingFile) {
    ...
    static def buildRouter(Vertx vertx, InputStream is) {
        if (!is) {
    ...
    static def buildRouter(Vertx vertx, Closure closure)  {
        RouterDSL routerDSL = new RouterDSL(vertx:vertx)
